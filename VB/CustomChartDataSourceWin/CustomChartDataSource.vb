...
Namespace DXSample
	Public Class CustomChartDataSource
		...
		Private Sub CreateChartData()
			If fPivot.OptionsSelection.MultiSelect AndAlso fPivot.OptionsChartDataSource.SelectionOnly Then
				fData = CreateChartDataFromMultiselection()
			Else
				fData = CreateChartDataFromSelection()
			End If
		End Sub
		...
		Private Function CreateChartDataFromSelection() As List(Of Object())
			...
			If fPivot.OptionsChartDataSource.SelectionOnly Then
				selection = fPivot.Cells.Selection
			Else
				selection = New Rectangle(0, 0, fPivot.Cells.ColumnCount, fPivot.Cells.RowCount)
			End If
		...
		Private Function GetDataFromCell(ByVal cell As PivotCellEventArgs) As Object()
			...
			If fPivot.OptionsChartDataSource.ChartDataVertical Then
				If fPivot.GetFieldsByArea(PivotArea.DataArea).Count > 1 Then
					result(0) = String.Concat(GetFieldValue(cell, False),String.Concat(" | ", cell.DataField.Caption))
				Else
					result(0) = String.Concat(GetFieldValue(cell, False),String.Empty)
				End If
			Else
				If fPivot.GetFieldsByArea(PivotArea.DataArea).Count > 1 Then
					result(0) = String.Concat(GetFieldValue(cell, True),String.Concat(" | ", cell.DataField.Caption))
				Else
					result(0) = String.Concat(GetFieldValue(cell, True),String.Empty)
				End If
			End If
			If fPivot.OptionsChartDataSource.ChartDataVertical Then
				result(1) = GetFieldValue(cell, True)
			Else
				result(1) = GetFieldValue(cell, False)
			End If
		...
		Private Shared Function GetFieldValue(ByVal cell As PivotCellEventArgs, ByVal column As Boolean) As String
			Dim fields() As PivotGridField
			If column Then
				fields = cell.GetColumnFields()
			Else
				fields = cell.GetRowFields()
			End If
		...
		Private Function SkipCellValue(ByVal cell As PivotCellEventArgs) As Boolean
            Return ((Not fPivot.OptionsChartDataSource.ShowColumnCustomTotals) AndAlso cell.ColumnValueType = PivotGridValueType.CustomTotal) OrElse _
            ((Not fPivot.OptionsChartDataSource.ShowColumnGrandTotals) AndAlso cell.ColumnValueType = PivotGridValueType.GrandTotal) OrElse _
            ((Not fPivot.OptionsChartDataSource.ShowColumnTotals) AndAlso cell.ColumnValueType = PivotGridValueType.Total) OrElse _
            ((Not fPivot.OptionsChartDataSource.ShowRowCustomTotals) AndAlso cell.RowValueType = PivotGridValueType.CustomTotal) OrElse _
            ((Not fPivot.OptionsChartDataSource.ShowRowGrandTotals) AndAlso cell.RowValueType = PivotGridValueType.GrandTotal) OrElse _
            ((Not fPivot.OptionsChartDataSource.ShowRowTotals) AndAlso cell.RowValueType = PivotGridValueType.Total)
		End Function
		...
		Private Event fListChanged As ListChangedEventHandler
		Protected Sub RaiseListChanged()
			RaiseEvent fListChanged(Me, New ListChangedEventArgs(ListChangedType.Reset, -1))
		End Sub
		Private Custom Event ListChanged As ListChangedEventHandler Implements IBindingList.ListChanged
			AddHandler(ByVal value As ListChangedEventHandler)
				AddHandler fListChanged, value
			End AddHandler
			RemoveHandler(ByVal value As ListChangedEventHandler)
				RemoveHandler fListChanged, value
			End RemoveHandler
		...
		Public Class CustomChartDataSourcePropertyDescriptor
			...
			Public Overrides Function GetValue(ByVal component As Object) As Object
				...
				End If
				Return source(CInt(Fix(fPropertyType)))
			End Function
			...
			Public Overrides ReadOnly Property PropertyType() As Type
				Get
					If fPropertyType = CustomChartDataSourceProperty.Values Then
